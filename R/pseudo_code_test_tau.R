...
osam=function(U,grad,pos,tau,h,w,m,m_sq,v_sq,cov_sq,deriv_sq,m_adam,v_adam,iter){
...
adaptive_malt=function(n,init){
  update=list(pos=init,tau=1.8,h=0.2,w=init,m=init,m_sq=sum(init^2),v_sq=0,cov_sq=0,deriv_sq=0,m_adam=0,v_adam=0,iter=1)
  eig_values=rep(NA,n)
  means=rep(NA,n)
  tau_values=rep(NA,n)
  chain=rep(NA,n)
  ...
  for(i in 1:n){
    update=osam(U,grad,update$pos,update$tau,update$h,update$w,update$m,update$m_sq,update$v_sq,update$cov_sq,update$deriv_sq,update$m_adam,update$v_adam,update$iter)
...
#plot(tau_average[1:5000],type="l")
#tau_average[5000]

#plot(output$rho_prime_values,type="l")
#plot(output$rho_prime_values[200:n],type="l")
